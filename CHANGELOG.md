...
- Ссылки на документацию tetiva.app/docs из приложения: пункт «Documentation» в настройках и контекстные «?» рядом с MCP, скриптами, gRPC-редактором, синхронизацией и окном окружений — каждый ведёт на свою страницу доков

## [v0.16.1] — 2026-08-12 — Безопасность MCP и песочницы скриптов
...
- **Скрипт из чужой коллекции больше не может подвесить приложение.** Регулярные выражения с обратными ссылками и просмотром вперёд (`/(a+)+\1$/`, `/^(?=(a+)+$)a*$/`) выполняются перебором с возвратами, и остановить их по таймауту нельзя. Теперь скрипт выполняется отдельно от запроса: через пять секунд запрос завершается ошибкой, окно и остальные вкладки продолжают работать
- gRPC-метаданные приходят в скрипт копией. `pm.request.metadata.set()` в pre-скрипте по-прежнему меняет метаданные отправляемого запроса, но сам запрос в коллекции остаётся нетронутым, а post-скрипт видит те метаданные, которые действительно ушли на сервер

### Исправлено

- `pm.request.metadata.set()` в запросе без метаданных мог уронить приложение
