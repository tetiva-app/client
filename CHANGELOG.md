...
## [v0.17.0] — 2026-08-16 — Устройства и лимиты тарифа
...
- Раздел «Devices» в окне синхронизации: устройства, где вы вошли в аккаунт, время последней активности, отметка текущего, выход с отдельного устройства и «Sign out everywhere»
- Уведомления о лимитах тарифа: если сервер отклонил синхронизацию из-за квоты (облачные коллекции, участники команды), приложение объясняет причину тостом и предлагает открыть страницу тарифов. Данные остаются на диске
- Состояние «Sync paused — plan limit reached» у индикатора синхронизации: при отказе по лимиту клиент повторяет попытки, а не считает сервер недоступным
- Сообщение о конфликте идентификаторов: если сущность уже принадлежит другому облачному воркспейсу, приложение говорит об этом вместо молчаливого пропуска
- Клиент представляется серверу User-Agent вида `Tetiva/версия (ос; хост)` — по нему в списке устройств видно, что это за устройство

### Изменено

- Выход из аккаунта завершает сессию и на сервере, не только на этом устройстве
- Лимит тарифа теперь виден постоянно: индикатор синхронизации показывает, сколько изменений не уехало в облако, а уведомление висит до закрытия и приходит один раз на эпизод, а не раз в пять минут

### Исправлено

- После перепривязки к другому облачному воркспейсу курсор синхронизации от прежнего больше не остаётся: содержимое нового загружается сразу
- Лимит тарифа определяется точно: раньше в него попадал любой отказ сервера с кодом `ResourceExhausted` — например, слишком большой пакет изменений — и синхронизация вставала «из-за тарифа», хотя дело было в другом
- Записи, отклонённые по квоте, больше не выбрасываются из очереди навсегда: они остаются в ней и повторяются раз в пять минут, так что после смены тарифа данные уезжают в облако сами
- Список устройств: во время загрузки видно «Loading devices», при одном устройстве — «No other devices», а серверный текст ошибки заменили понятное сообщение и кнопка «Retry». Кнопки выхода блокируются на время запроса
- Устройство в списке подписано как «хост · ОС · версия» вместо голого User-Agent; у сессии без активности время последней активности больше не показывается вместо «NaN»
- Индикатор синхронизации при лимите тарифа из серого стал янтарным: остановку видно сразу
- Выход из аккаунта во время фонового обновления токена больше не оставляет чужой refresh-токен в базе
- Запросы к серверу для списка устройств, выхода с устройства и выхода отовсюду ограничены по времени и не подвешивают окно при недоступном сервере
- Повторный отказ по лимиту тарифа снова показывает уведомление: раньше признак «лимит уже показан» сбрасывался только успешной отправкой, и клиент, восстановившийся с пустой очередью, о следующем лимите молчал
- Записи, отложенные из-за квоты, возвращаются в очередь сами через пять минут — раньше их ждало пробуждение только при следующей локальной правке или переподключении
- Отложенные из-за лимита тарифа записи повторяются и после перезапуска приложения: пробуждение жило только в памяти процесса, поэтому после рестарта они ждали ближайшей локальной правки в этом воркспейсе
- Счётчик неотправленных изменений учитывает записи, отложенные до повторной попытки: они больше не показываются как «всё отправлено»
- Окно синхронизации: подпись устройства больше не обрезается на версии (время последней активности ушло на вторую строку), содержимое подключённого состояния не вылезает за правый край окна, а обрыв связи с бэкендом больше не оставляет кнопку в состоянии «Connecting...» навсегда
- Окно синхронизации объясняет, почему изменения не уезжают в облако: при упоре в лимит тарифа появляется янтарная плашка с причиной и ссылкой на тарифы, а само окно больше не подпрыгивает при открытии
- Старт приложения при недоступном сервере больше не глушит синхронизацию: движок стартует офлайн и переподключается сам
